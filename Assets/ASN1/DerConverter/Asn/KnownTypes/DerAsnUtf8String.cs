...
namespace DerConverter.Asn.KnownTypes
{
    public class DerAsnUtf8String : DerAsnType<string>
    {
        ...
        protected override string DecodeValue(IDerAsnDecoder decoder, Queue<byte> rawData) => Encoding.UTF8.GetString(rawData.DequeueAll().ToArray());

        protected override IEnumerable<byte> EncodeValue(IDerAsnEncoder encoder, string value) => Encoding.UTF8.GetBytes(value);
    }
}
